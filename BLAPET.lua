-- // ZHXHub Full Project v1.5 — Fix: triggerPetBug identik dengan Blapet Auto v6.5
...
-- Auto Event
-- ========== RADAR AUTO EVENT (V1.5) ==========
local function findEventModel(eventName)
    -- Ambil kata kunci utama, contoh: dari "Megalodon Hunt" jadi "Megalodon"
    local baseWord = eventName:match("%w+") or eventName 
    
    local function isMatch(obj)
        -- Jangan sampai salah deteksi karakter player yang namanya mengandung Megalodon
        if obj:FindFirstChildOfClass("Humanoid") and Players:GetPlayerFromCharacter(obj) then return false end
        
        -- Cek jika namanya sama persis ATAU mengandung kata "Megalodon"
        local matchName = (obj.Name == eventName or string.find(obj.Name, baseWord))
        local matchClass = (obj:IsA("Model") or obj:IsA("Folder") or obj:IsA("BasePart"))
        return matchName and matchClass
    end

    -- Sapu bersih semua folder yang biasa dipakai untuk menaruh event
    local foldersToCheck = {
        WS:FindFirstChild("active_events"),
        WS:FindFirstChild("ActiveEvents"),
        WS:FindFirstChild("Zones"),
        WS:FindFirstChild("Props"),
        WS:FindFirstChild("Map"),
        WS -- Pilihan terakhir: sapu langsung di luar Workspace
    }

    for _, folder in ipairs(foldersToCheck) do
        if folder then
            for _, child in ipairs(folder:GetChildren()) do
                if isMatch(child) then return child end
            end
        end
    end
    return nil
...
local function getEventPosition(eventName)
    local eventObj = findEventModel(eventName)
    if not eventObj then return nil end

    -- 1. Jika objeknya sendiri adalah Part pijakan
    if eventObj:IsA("BasePart") then return eventObj.Position + Vector3.new(0, 5, 0) end
    
    -- 2. Jika objeknya Model dan punya Part utama
    if eventObj:IsA("Model") and eventObj.PrimaryPart then return eventObj.PrimaryPart.Position + Vector3.new(0, 5, 0) end

    -- 3. Korek ke dalam isi folder/model untuk cari Part yang bisa diinjak
    for _, child in ipairs(eventObj:GetDescendants()) do
        if child:IsA("BasePart") then return child.Position + Vector3.new(0, 5, 0) end
    end
    
    -- 4. Pilihan terakhir (Fallback hitung area 3D)
    if eventObj:IsA("Model") then
        local cf = eventObj:GetBoundingBox()
...
print("ZHXHub Full Project v1.5.")
